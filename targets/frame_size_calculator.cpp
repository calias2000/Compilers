...
void fir::frame_size_calculator::do_corpo_node(fir::corpo_node *const node, int lvl) {
  if (node->prologo()) node->prologo()->accept(this, lvl + 2);
  if (node->bloco()) node->bloco()->accept(this, lvl + 2);
  if (node->epilogo()) node->epilogo()->accept(this, lvl + 2);
}

void fir::frame_size_calculator::do_bloco_node(fir::bloco_node *const node, int lvl) {
  if (node->declarations()) node->declarations()->accept(this, lvl + 2);
  if (node->instructions()) node->instructions()->accept(this, lvl + 2);
}

void fir::frame_size_calculator::do_prologo_node(fir::prologo_node *const node, int lvl) {
  if (node->declarations()) node->declarations()->accept(this, lvl + 2);
  if (node->instructions()) node->instructions()->accept(this, lvl + 2);
}
